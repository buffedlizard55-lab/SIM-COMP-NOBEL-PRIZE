...
| `batches/batch-001.json` … `batch-020.json` | Batch report: topic statement, full 100-row leaderboard per universe, parameter sensitivity, null band |
| `notes.json` | Decision-note codes that occurred, per family, with counts |
| `activity.json` | Per universe, per UTC day: number of ledger rows across all 2,000 participants |
| `research.json` | Family-level research results: research cards, information channels, verified sources, per-universe statistics and verdicts, verdict rules, multiple-testing count, data-quality flags, replay status (see below) |
| `ledger_hashes.json` | Per universe, per participant: sha-256 of that participant's ledger; `verify_program.py` compares against it |
...
4. Replay any batch: `python scripts/verify_program.py batch-004` re-runs that
   ...
   against `manifest.json`. `global` replays all twenty batches (slow).
5. The universe-level check is `manifest.json → universes → {universe} →
   ...
   The replay proof in `manifest.json → replay` shows batches 001, 011, 013 and
   018 were re-run after the full pass and matched.

## research.json

Generated by `simcomp/analysis.py` on every build. Nothing in it is a forecast.
`docs/RESEARCH.md` is the same content rendered as Markdown.

Top-level keys:

- `simulated`: always `true`.
- `rules`: verdict name → rule text.
- `min_events`: settled events a family's fills must touch.
- `channels`: channel id → what it contains at T.
- `sources` / `sources_verified_on`: citation, URL or DOI, and the claim relied on.
- `universes`: per universe, the event and market counts (markets with decision
  candles), the batch-001 null band (`n`, `p05`, `median`, `p95`) and the
  top-decile cut.
- `families`: one entry per family (see below).
- `overall_counts`: count of `overall` verdicts.
- `multiple_testing`: tests that reached a verdict and the false passes expected
  at 5%.
- `data_quality`: `code`, `severity`, `message`.
- `replay`: `status` plus the batches that were re-run.

Each `families[]` entry has:

- `family`, `batches`, `variants`;
- `card` (`question`, `mechanism`, `channels`, `predicts`, `falsified_if`,
  `sources`, `kind`);
- `universes`, with one block per universe (below);
- `overall`: the combined verdict across the settled universes;
- `consistency`: the direction of the traded median against the null median in
  each settled universe.

Per-universe fields:

| Field | Meaning |
| --- | --- |
| `n` / `entered` | Variants in the family / variants with at least one fill |
| `median_eq` | Median ending equity of the variants that traded (used by the verdict) |
| `median_eq_all_variants` | Median over all variants, including those that never traded |
| `vs_cash`, `vs_null_median` | `median_eq` − $10,000 and − the null median |
| `p05_eq`, `p95_eq` | Spread of traded variants |
| `share_above_null_p95`, `binom_p_indep` | Share of traded variants above the null p95. The binomial p-value assumes independent variants, so it overstates the evidence. |
| `p_top_decile` | Share of all the family's variants in the universe's top decile of all 2,000 participants. Used by the P(Win) card on settled universes only. |
| `placebo_family`, `placebo_median_eq`, `vs_placebo` | Matched placebo, if any |
| `events_touched`, `settled_events_touched`, `yes_events_touched` | Independent outcomes behind the result |
| `top_event`, `top_event_share` | Event with the largest absolute P&L share |
| `loo_worst_median_eq`, `loo_worst_event` | Worst family median after dropping one event's P&L |
| `fills`, `clip_rate` | Filled orders; share whose size the engine clipped |
| `verdict`, `why` | Verdict and the numbers that produced it |
